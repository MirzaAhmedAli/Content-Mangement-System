...
class ForgotPasswordController extends Controller
{
    ...
    public function forgotPasswordPost(ForgetPasswordRequest $request){
         ...
         Mail::send('pages.emails.forgot-password-email', ['token' => $token], function($message) use($request){
             $message ->to($request->email);
             $message ->subject('Reset Password');
         }); 
        return redirect('/login')->with('status','An Email has been sent to you. Please check your Inbox');
    }
