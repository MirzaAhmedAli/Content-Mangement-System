...
<nav class=" border-gray-200 shadow-lg">
  ...
  <div class="editor max-w-4xl px-6 py-2 mx-auto w-11/12 flex flex-col text-gray-800 border border-gray-300 rounded-lg p-4 shadow-lg justify-start text-left">
      <div class="heading text-left font-bold text-3xl m-5 text-yellow-600 font-serif">Edit Post</div>
      
      <input class="title bg-gray-100 border border-gray-300 rounded-lg p-2 mb-4 outline-none" spellcheck="false" placeholder="Title" type="text" name="title" value="{{$post->title}}">
      @error('title') <span class="text-danger text-red-700">{{ $message }}</span> @enderror
      
      <textarea class="description bg-gray-100 sec p-3 h-60 border rounded-lg border-gray-300 outline-none" spellcheck="false" placeholder="Rev your thoughts here..." name="description">{{ $post->description }}</textarea>
                  ...
                  @foreach ($categories as $category)
                      <li>
                          <label class="flex items-center w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                              <input type="radio" name="category_id" value="{{ $category->id }}" class="mr-2" {{ $post->category_id == $category->id ? 'checked' : '' }} onclick="showSubcategories({{ $category->id }})">
                              {{ $category->name }}
                                  ...
                                  @foreach ($category->subcategories as $subcategory)
                                      <li>
                                          <label class="flex items-center px-4 py-2">
                                              <input type="radio" name="subcategory_id" value="{{ $subcategory->id }}" class="mr-2" {{ $post->subcategory_id == $subcategory->id ? 'checked' : '' }}>
                                              {{ $subcategory->name }}
                                          </label>
                                      </li>
                                  @endforeach
      ...
      @error('tags') <span class="text-danger text-red-700">{{ $message }}</span> @enderror
      ...
      <div class="buttons flex">
          ...
          <div class="btn border rounded-lg border-indigo-500 p-1 px-4 font-semibold cursor-pointer text-gray-200 ml-2 bg-indigo-500"><button type="submit">Update</button></div>
      </div>
  </div>
</form>
