...
<section class="relative w-full min-h-screen bg-cover bg-center mt-4 text-teal-600 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700">
  ...
  <div class="relative p-5 lg:px-20 flex flex-col md:flex-row items-center justify-center mt-20">
  
      <form action="{{route('store.subcategory')}}" class="w-full md:w-1/2 border border-red-500 p-6 bg-gray-900" method="POST">
      @csrf
      ...
      <div>
          ...
          <input 
              type="text" name="name" 
              class="px-3 py-2 bg-gray-800 border border-gray-900 focus:border-red-500 focus:outline-none focus:bg-gray-800 focus:text-white">
              @error('name') <span class="text-danger text-red-700">{{$message}}</span> @enderror
            </div>
          <div class="flex flex-col mb-3">
          <label>Description</label>
          <input type="text" class="px-3 py-2 bg-gray-800 border border-gray-900 focus:border-red-500 focus:outline-none focus:bg-gray-800 focus:text-white" name="description"> 
      ...
      <div class="w-full pt-3">
          <button type="submit" class="w-full bg-gray-900 border rounded-lg border-white px-4 py-2 transition duration-50 focus:outline-none font-semibold hover:bg-red-500 hover:text-white text-xl cursor-pointer">
          Create
          </button>
      </div>
      </form>
